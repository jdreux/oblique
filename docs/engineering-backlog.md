...
2. After shader compilation succeeds and before the uniform loop, if `_debug_mode`:
   - Get the set of uniform names from the compiled program (moderngl `Program` supports `__contains__` and iteration).
...
### B1. Remove `width`/`height` from all module Params dataclasses

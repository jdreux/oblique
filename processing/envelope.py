...
class Envelope(BaseProcessingOperator[float]):
    """
    ...
    metadata: Dict[str, Any] = {
        ...
        "parameters": {"decay": float},
    }

    def __init__(self, input_fn: Callable[[], float], decay: float = 0.1):
        super().__init__()
    ...
    def process(self) -> float:
        x = self.input_fn()
        self.value = self.decay * x + (1 - self.decay) * self.value
        return self.value
...
if __name__ == "__main__":  # pragma: no cover
    ...
    env = Envelope(input_fn, decay=0.2)
    for _ in range(100):
        print(env.process())
